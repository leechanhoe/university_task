...
qual_lo = fuzz.trimf(x_qual, [0, 0, 10])
qual_md = fuzz.trimf(x_qual, [0, 5, 10])
qual_hi = fuzz.trimf(x_qual, [0, 10, 10])
serv_lo = fuzz.trimf(x_serv, [0, 0, 7])
serv_md = fuzz.trimf(x_serv, [0, 6, 10])
serv_hi = fuzz.trimf(x_serv, [4, 10, 10])
tip_lo = fuzz.trimf(x_tip, [0, 0, 13])
tip_md = fuzz.trimf(x_tip, [0, 8, 25])
tip_hi = fuzz.trimf(x_tip, [18, 25, 25])
...
progress = []
for food in range(0, 11):
    for serv in range(0, 11):
        # We need the activation of our fuzzy membership functions at these values.
        # The exact values 6.5 and 9.8 do not exist on our universes...
        # This is what fuzz.interp_membership exists for!
        qual_level_lo = fuzz.interp_membership(x_qual, qual_lo, food) # 그래프에서 어느 한 점의 값
        qual_level_md = fuzz.interp_membership(x_qual, qual_md, food)
        qual_level_hi = fuzz.interp_membership(x_qual, qual_hi, food)
        ...
        tip = fuzz.defuzz(x_tip, aggregated, 'bisector') # 비퍼지화
        progress.append(tip)
...
x = np.tile(xx, (11,1))
y = np.transpose(x)
z = np.array(progress).reshape((11, 11))
print(x)
print(y)
print(z)
# for i in range(11):
